...
| **Generated results.csv** | ✅ Generated by `scripts/run_experiments.py`; rerun after code changes |
...
The results.csv schema gains one column: `run_id` in Sprint 1, and later adds `tpr_mia_knn_ratio` for the second MIA baseline. Rows grow from 9 to 41
...
**Implemented output schema:** Sprint 3 adds `probe_rouge_l_mean`, and the k-NN MIA extension adds
`tpr_mia_knn_ratio` to `data/results.csv`, so the expected full CSV has 61 rows and 10 columns
...
Run `pytest tests/ -v` after every change. All tests must pass. If your
...
`python scripts/run_experiments.py`. If you need to rerun, do so fully. The k-NN MIA column is also generated by this script and should not be manually added to an old CSV.
...
pytest tests/ -v                # All tests must pass before proceeding
...
### k-NN Distance-Ratio MIA Baseline

**Status:** ✅ Implemented for the no-noise Baseline plus Central, Local, and Metric DP conditions.

A lightweight second membership-inference baseline is available in
`src/evaluation/mia_knn_ratio.py`. It fits two auxiliary k-NN banks from the
shadow member and shadow non-member embeddings, then scores each target vector by
`log((d_non_member + eps) / (d_member + eps))`. Larger scores are more
member-like. `scripts/run_experiments.py` writes the result to the new
`tpr_mia_knn_ratio` column using the same TPR @ 0.1% FPR protocol as LiRA.

The `tpr_mia_knn_ratio` column is now populated for every evaluated mechanism:
Baseline, Central DP, Local DP, and Metric DP. Re-running
`python scripts/plot_results.py` creates
`docs/figures/mia_knn_ratio_vs_epsilon.png`, with all DP mechanisms shown on
the same epsilon sweep.
