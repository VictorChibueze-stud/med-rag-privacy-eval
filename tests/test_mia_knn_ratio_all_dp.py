...
def test_knn_ratio_runs_on_baseline_central_local_and_metric_dp() -> None:
    ...
    labels = np.concatenate([np.ones(10, dtype=np.int64), np.zeros(10, dtype=np.int64)])
    clean_corpus = np.vstack([shadow_members, shadow_non_members])
    attack = KNNDistanceRatioMembershipInference(k=5).fit(shadow_members, shadow_non_members)
    central = CentralDPMechanism(epsilon=1.0, delta=1e-05).apply_noise(target)
    local = LocalDPProjector(input_dim=384, bottleneck_dim=16, epsilon=1.0, delta=1e-05)
    local.eval()
    with torch.no_grad():
        local_dp = local(torch.from_numpy(target.astype(np.float32))).numpy()
    metric = MetricDPMechanism(epsilon=1.0, delta=1e-05, k=5).apply_noise(target, clean_corpus)
    outputs = {'Baseline': target, 'Central': central, 'Local': local_dp, 'Metric': metric}
