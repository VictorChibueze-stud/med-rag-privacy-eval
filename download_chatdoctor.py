...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Download ChatDoctor from Hugging Face to data/chatdoctor.json.')
    parser.add_argument('--dataset', default=DEFAULT_DATASET, help=f'Dataset name (default: {DEFAULT_DATASET})')
    parser.add_argument('--split', default=DEFAULT_SPLIT, help=f'Split to download (default: {DEFAULT_SPLIT})')
    parser.add_argument('--output', type=Path, default=DEFAULT_OUTPUT, help=f'Output path (default: {DEFAULT_OUTPUT})')
    parser.add_argument('--limit', type=int, default=None, help='Optional row cap before saving.')
...
def import_load_dataset():
    try:
        ...
        raise SystemExit('Missing dependency: datasets\n\nInstall it with:\n    python -m pip install datasets\n\nThen run again:\n    python download_chatdoctor.py') from exc
    return load_dataset

def normalise_row(row: dict[str, Any]) -> dict[str, str]:
    return {'instruction': str(row.get('instruction', '')).strip(), 'input': str(row.get('input', '')).strip(), 'output': str(row.get('output', '')).strip()}
