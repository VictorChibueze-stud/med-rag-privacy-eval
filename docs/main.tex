...
We presented the first empirical three-way comparison of Central, Local, and
Metric differential privacy for sentence embeddings in a medical RAG pipeline,
evaluated across membership inference, embedding inversion, and semantic utility
axes on 5,000 patient consultation records. Central and Local DP exhibit
epsilon-invariant protection: inversion ROUGE-L drops immediately to $\approx
0.21$ and remains flat across $\varepsilon \in [0.1, 10]$, while BERTScore F1
stays above $0.83$ --- less than 2\% utility degradation at even the strongest
privacy budget. Metric DP offers modestly better utility at low $\varepsilon$
but collapses at $\varepsilon \geq 5$, reaching inversion ROUGE-L of $0.950$
at $\varepsilon = 10$. For medical RAG deployments, we recommend Central DP:
the choice of $\varepsilon$ within $[0.1, 10]$ does not affect protection,
giving practitioners operational freedom without sacrificing privacy guarantees.
Future work should evaluate encoder-level privacy via DP-SGD fine-tuning and
extend the inversion evaluation to Vec2Text-strength attacks on clinical corpora.
