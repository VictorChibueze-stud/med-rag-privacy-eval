...
class LiRAMembershipInference:
    def __init__(self, n_shadow_models: int=16) -> None:
        ...
        self.shadow_models: list[LogisticRegression] = [LogisticRegression(max_iter=1000) for _ in range(n_shadow_models)]
    ...
    def train_shadow_models(self, shadow_member_embeddings: np.ndarray, shadow_non_member_embeddings: np.ndarray) -> None:
        ...
        if x_m.shape[0] < self.n_shadow_models:
            msg = f'shadow_member_embeddings has {x_m.shape[0]} rows but n_shadow_models={self.n_shadow_models}. Need at least n_shadow_models rows to avoid degenerate bootstrap resamples.'
            raise ValueError(msg)
        ...
        for model in self.shadow_models:
            ...
            y_boot = np.concatenate([np.ones(len(idx_m), dtype=np.int64), np.zeros(len(idx_n), dtype=np.int64)])
        ...
        if self.sigma_out < 1e-08:
            self.sigma_out = 1e-08

    def evaluate_tpr_at_fpr(self, target_embeddings: np.ndarray, target_labels: np.ndarray, fpr_threshold: float=0.001) -> float:
        ...
        if x.ndim != 2 or x.shape[0] != labels.shape[0]:
            ...
            raise ValueError(msg)
        avg_probs = np.mean(np.stack([m.predict_proba(x)[:, 1] for m in self.shadow_models], axis=0), axis=0)
