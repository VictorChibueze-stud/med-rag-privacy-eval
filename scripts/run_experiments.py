...
logging.basicConfig(level=logging.INFO, format='%(asctime)s | %(levelname)s | %(message)s', datefmt='%H:%M:%S', handlers=[logging.StreamHandler(), logging.FileHandler('data/experiment.log', mode='w', encoding='utf-8')])
...
def _encode_corpus(model: RAGBaseline, texts: list[str], label: str) -> np.ndarray:
    ...
    arr = model.model.encode(list(texts), normalize_embeddings=True, show_progress_bar=False)
    log.info('Encoding finished | phase=%s | texts=%d', label, len(texts))
    return np.asarray(arr, dtype=np.float32)

def _eval_mechanism(embs: np.ndarray, target_labels: np.ndarray, orig_by_row: list[str], sample_i: np.ndarray, lira: LiRAMembershipInference, knn_ratio: KNNDistanceRatioMembershipInference | None, inv: EmbeddingInversion, probe: LinearProbeInversion, util: UtilityEvaluator, label: str) -> dict[str, float]:
    ...
    return {'tpr_mia': tpr, 'tpr_mia_knn_ratio': tpr_knn_ratio, 'inversion_rouge_l_mean': mean_rouge, 'probe_rouge_l_mean': mean_probe_rouge, 'bert_precision': bs['precision'], 'bert_recall': bs['recall'], 'bert_f1': bs['f1']}
...
def main() -> None:
    ...
    target_texts, shadow_texts = train_test_split(all_texts, test_size=0.3, random_state=42, shuffle=True)
    sm_text, snm_text = train_test_split(shadow_texts, test_size=0.5, random_state=11, shuffle=True)
    tm_text, tnm_text = train_test_split(target_texts, test_size=0.5, random_state=22, shuffle=True)
    rag = RAGBaseline()
    ...
    target_labels = np.concatenate([np.ones(e_tm.shape[0], dtype=np.int64), np.zeros(e_tnm.shape[0], dtype=np.int64)])
    ...
    sample_i = rng.choice(target_embs.shape[0], size=min(100, target_embs.shape[0]), replace=False)
    text_tm = list(tm_text)
    text_tnm = list(tnm_text)
    orig_by_row = text_tm + text_tnm
    inv = EmbeddingInversion(line_texts, np.ascontiguousarray(all_clean, dtype=np.float32))
    ...
    baseline_metrics = _eval_mechanism(baseline_embs, target_labels, orig_by_row, sample_i, lira, knn_ratio, inv, probe, util, 'Baseline')
    rows.append({'run_id': 0, 'epsilon': float('inf'), 'mechanism': 'Baseline', **baseline_metrics})
    ...
    for run_id in range(N_RUNS):
        ...
        for eps in EPSILONS:
            log.info('Evaluation starting | run_id=%d | epsilon=%s | mechanism=Central', run_id, eps)
            central = CentralDPMechanism(epsilon=float(eps), delta=1e-05)
            noisy_c = np.asarray(central.apply_noise(np.asarray(target_embs, dtype=np.float64)), dtype=np.float32)
            central_metrics = _eval_mechanism(noisy_c, target_labels, orig_by_row, sample_i, lira, knn_ratio, inv, probe, util, f'run_id={run_id} epsilon={eps} mechanism=Central')
            rows.append({'run_id': run_id, 'epsilon': eps, 'mechanism': 'Central', **central_metrics})
            log.info('Evaluation finished | run_id=%d | epsilon=%s | mechanism=Central', run_id, eps)
            log.info('Evaluation starting | run_id=%d | epsilon=%s | mechanism=Local', run_id, eps)
            local = LocalDPProjector(input_dim=384, bottleneck_dim=16, epsilon=float(eps), delta=1e-05)
            local.eval()
            with torch.no_grad():
                t_in = torch.from_numpy(np.ascontiguousarray(target_embs, dtype=np.float32))
                noisy_l = local(t_in).numpy().astype(np.float32)
            local_metrics = _eval_mechanism(noisy_l, target_labels, orig_by_row, sample_i, lira, knn_ratio, inv, probe, util, f'run_id={run_id} epsilon={eps} mechanism=Local')
            rows.append({'run_id': run_id, 'epsilon': eps, 'mechanism': 'Local', **local_metrics})
            log.info('Evaluation finished | run_id=%d | epsilon=%s | mechanism=Local', run_id, eps)
            log.info('Evaluation starting | run_id=%d | epsilon=%s | mechanism=Metric', run_id, eps)
            metric = MetricDPMechanism(epsilon=float(eps), delta=1e-05, k=20)
            noisy_m = np.asarray(metric.apply_noise(np.asarray(target_embs, dtype=np.float64), np.asarray(all_clean, dtype=np.float64)), dtype=np.float32)
            metric_metrics = _eval_mechanism(noisy_m, target_labels, orig_by_row, sample_i, lira, knn_ratio, inv, probe, util, f'run_id={run_id} epsilon={eps} mechanism=Metric')
            rows.append({'run_id': run_id, 'epsilon': eps, 'mechanism': 'Metric', **metric_metrics})
            log.info('Evaluation finished | run_id=%d | epsilon=%s | mechanism=Metric', run_id, eps)
    out_dir = _ROOT / 'data'
    out_dir.mkdir(parents=True, exist_ok=True)
    df = pd.DataFrame(rows)
    df = df[['run_id', 'epsilon', 'mechanism', 'tpr_mia', 'tpr_mia_knn_ratio', 'inversion_rouge_l_mean', 'probe_rouge_l_mean', 'bert_precision', 'bert_recall', 'bert_f1']]
    ...
    log.info('Experiment script finished | elapsed=%dm %02ds', elapsed_minutes, elapsed_seconds)
